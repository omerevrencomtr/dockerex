...
@section('title') Kripto Varlık Yönetimi @endsection
@section('content')
            ...
            <div class="row">
                ...
                <div class="col-sm-12 col-md-4 col-lg-4 col-xl-4">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <span>Kripto koin aktarımlarıyla ilgili dikkat edilmesi gerekenler;</span>
                            <ul>
                                <li>Adresler hesap <u>bakiyenize özel</u> tanımlanmıştır.</li>
                                <li>Aktarımlar gerekli <u>onay sayısına</u> ulaştığında hesabınıza bakiyelendirilir.
                                </li>
                                <li>Tanımlı adresinizi <u>en az bir kez</u> kullandıktan sonra yeni adres
                                    oluşturabilirsiniz.
                                </li>
                                <li>Kripto koin çekme emirleri, Tesst onayıyla birlikte bakiyenizden düşülür.</li>
                                <li>Aktarım süreleri ilgili kripto birimin blok zincirindeki yoğunluğa göre
                                    değişebilir.
                                </li>
                                <li>Adreslerinizi <u>ödeme almak için</u> kullanabilirsiniz.</li>
                                <li>Minimum işlem limitleri ve onay sayıları aşağıdaki gibidir:</li>
                                    ...
                                    <form>
                                        <div class="input-group mb-3">
                                            <div class="input-group-prepend">
                                                <span class="input-group-text"><i id="depositLogo"
                                                                                  class="cf cf-btc"></i></span>
                                            </div>
                                            <input id="depositAddress" type="text" readonly="readonly"
                                                   class="form-control address">
                                            <div class="input-group-prepend">
                                                <button id="depositAddressCopy" data-clipboard-target="#depositAddress"
                            ...
                            <div style="display: none;" id="withdrawRow" class="row">
                                <div class="col-md-12">
                                    <form id="withdrawForm">
                                        ...
                                        <div class="input-group mb-3">
                                            <div class="input-group-prepend">
                                                <span class="input-group-text"><span id="withdrawName"></span>&nbsp;Adres</span>
                                            </div>
    ...
    <div class="row">
        <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
            <div class="card">
                ...
                <div class="card-body">
                    <table id="financialCryptoHistoryTable" width="100%"
                           class="table table-hover table-striped table-bordered dt-responsive">
                        <thead>
                        <tr>
                            <th>İşlem tarihi</th>
...
@section('script')
    ...
    <script>
        var historyTable = $('#financialCryptoHistoryTable').DataTable({
            "language": {
            ...
            "serverSide": true,
            "ajax": {
                "url": "{{route('dashboard.user.balance.crypto.getHistory')}}",
                "dataType": "json",
                "type": "POST",
        ...
        var pendingTable = $('#financialCryptoPendingTable').DataTable({
            "language": {
                "url": "//cdn.datatables.net/plug-ins/1.10.12/i18n/Turkish.json"
            },
            "visible": true,
            ...
            "serverSide": true,
            "ajax": {
                "url": "{{route('dashboard.user.balance.crypto.getPending')}}",
                "dataType": "json",
                "type": "POST",
                "headers": {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            },
    ...
    <script type="text/javascript">
        ...
        function withdrawCancel(id) {
            $.ajax({
                type: "POST",
                url: '{{route('dashboard.user.balance.crypto.withdrawCancel')}}',
                data: {'id': id},
                headers: {
        ...
        $(document).ready(function () {
            ...
            $('#withdrawFinalButton').click(function () {
                $.ajax({
                    type: "POST",
                    url: '{{route('dashboard.user.balance.crypto.withdraw')}}',
            ...
            $('#withdrawConfirmButton').click(function () {
                ...
                if (parseFloat(balance) >= parseFloat(amount)) {
                    ...
                    $.ajax({
                        ...
                        type: "POST",
                        url: '{{route('dashboard.user.balance.crypto.getWithdrawConfirm')}}',
        ...
        function withdraw(name) {
            ...
            $.ajax({
                ...
                type: "POST",
                url: '{{route('dashboard.user.balance.crypto.getwithdraw')}}',
                data: {'name': name},
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
        ...
        function deposit(name) {
            $.ajax({
                ...
                type: "POST",
                url: '{{route('dashboard.user.balance.crypto.deposit')}}',
                data: {'name': name},
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
